...
int MuesliRunPythonV2(NSString *runtime, NSString *operation, NSString *source, NSString *model,
                   NSString *stream, NSString *component, NSString *sourceID, NSString *leaseToken,
    ...
    if (!leaseToken.length || leaseToken.length > 4096 || setenv("MUESLI_MEETING_LEASE", leaseToken.UTF8String, 1)) return 66;
...
#define API(name) __typeof__(name) *fn_##name = dlsym(python, #name); if (!fn_##name) return 70
    ...
    if (status._type != 0) { fn_PyConfig_Clear(&config); return 70; }
    char *arguments[] = {"muesli-inference", (char *)operation.UTF8String, (char *)source.fileSystemRepresentation, (char *)model.fileSystemRepresentation, (char *)stream.UTF8String, (char *)component.UTF8String, (char *)sourceID.UTF8String};
    status = fn_PyConfig_SetBytesArgv(&config, 7, arguments);
    if (status._type != 0) { fn_PyConfig_Clear(&config); return 70; }
