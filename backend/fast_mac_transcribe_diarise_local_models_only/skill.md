...
description: Transcribe audio/video files. FAST - 58x realtime on Apple Silicon. Optionally adds speaker diarization (who said what) and speaker name identification from context clues. Uses Parakeet MLX + Senko CoreML. Accepts local files or YouTube URLs.
---
...
**ALWAYS ask the user two questions using AskUserQuestion:**

**Question 1: Diarization**
- Header: "Speakers"
- Question: "Do you want speaker diarization (who said what)?"
- Options:
  - "Yes - separate speakers" (adds SPEAKER_01, SPEAKER_02 labels)
  - "No - plain transcript" (just the text, no speaker labels)

**Question 2: Speaker Identification** (only if diarization = yes)
- Header: "Identify"
- Question: "Should I try to identify speaker names using context clues?"
- Options:
  - "Yes - use context" (extracts video frames for name tags, analyzes text for introductions/mentions)
  - "No - keep as SPEAKER_XX" (just use generic labels)
...
Use `fast-transcribe` for most transcription needs - it's fast enough for any file length and accurate enough for general use. The user chooses whether they need diarization and speaker identification via the AskUserQuestion prompts.
